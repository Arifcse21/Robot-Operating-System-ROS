...
class MotorDriver(object):
    def __init__(self, wheel_distance = 0.115, wheel_diameter =  0.063, i_BASE_PWM = 50, i_MULTIPLIER_STANDARD = 0.1, i_MULTIPLIER_PIVOT = 1.0, simple_mode = True):
        """
        M1 = Right Wheel
        M2 = Left Wheel
    ...
    def calculate_wheel_turn_radius(self, body_turn_radius, angular_speed, linear_speed):
    ...
    def calculate_wheel_rpm(self, linear_speed, angular_speed, wheel_turn_radius):
        ...
        :param angular_speed:
        :paran wheel_turn_radius:
        :return:
        """
        if wheel_turn_radius is not None:
            # The robot is turnong
    ...
    def set_wheel_movement(self, right_wheel_rpm, left_wheel_rpm):
        # print("W1,W2=["+str(right_wheel_rpm)+","str(left+_wheel_rpm)+"]")
        ...
        # print("All forward")
            ...
            if self.simple_mode:
                # we make it turn only on 2 wheel
			...
			# print("Right Wheel forward, left stop")
			self.set_M1M2_speed(abs(right_wheel_rpm), abs(left_wheel_rpm), self.MULTIPLIER_STANDARD)
			self.left()
			...
			if self.simple_mode:
				# "We make it run only on 2 wheel"
	...
	def set_cmd_vel(self, linear_speed, angular_speed):
		...
		wheel = "right"
		right_wheel_turn_radius = self.calculate_body_turn_radius(body_turn_radius, angular_speed, wheel)
		
		wheel = "left"
		left_wheel_turn_radius = self.calculate_body_turn_radius(body_turn_radius, angular_speed, wheel)
